with care_mapping as (
        ...
        select
            ...
            paid_amount,
            case
                when encounter_type ilike '%inpatient%' then 'Inpatient'
                when encounter_type ilike '%ER%' or encounter_type ilike '%emergency%' then 'ER'
                when encounter_type ilike '%outpatient%' then 'Outpatient'
                else 'other'
            end as care_setting,
